...
function readSystemInformation(){
    ...
    systeminformation.GraphicCards.forEach((cardObject) => document.getElementById("writeDataGPU").innerHTML += `<li>${cardObject.model}</li>`);
    // for(i = 0; i < systeminformation.GraphicCards.length; i++) {
    //     document.getElementById('writeDataGPU').innerHTML = systeminformation.GraphicCards[i].model;
    // }
...
if (document.getElementById('writeDataGPU').innerHTML.indexOf("NVIDIA") != -1) {document.getElementById("NVIDIA").style.display = "block";}
